...
(function() {
  ...
  class Host {
    constructor() {
      ...
      this.emotions = {
        ...
        'already-flipped': ['smiling-face', 'smiling-eyes'],
      ...
      this.messages = {
        ...
        'already-flipped': [
          'Already flipped that one!',
          'Come on, select another card!',
  ...
  class MemoryGame {
    constructor() {
      // get number of cards from data-cards
      let boardElement = document.querySelector('.board');
      let cards = 0;
      if (boardElement) {
        cards = boardElement.getAttribute('data-cards');
      }

      this.totalCards = parseInt(cards, 10);
    ...
    checkFound(cardId) {
      if (this.flippedCard) {
        ...
        if (this.flippedCard !== cardId && flippedImg === cardImage) {
          ...
          let cardsFound = document.querySelectorAll('.card--found').length;

          // game finished: victory
          ...
          ((memoryGame, card1, card2) => setTimeout(() => {
            if (card1) {
              let card1Element = document
                .querySelector('[data-unique="' + card1 + '"]');
            ...
            if (card2) {
              let card2Element = document
                .querySelector('[data-unique="' + card2 + '"]');
